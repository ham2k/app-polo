...
import { motaFindOneByReference } from './MOTADataFile'
import { fmtDistance } from '../../../tools/geoTools'
import { Ham2kListItem } from '../../../screens/components/Ham2kListItem'

export function MOTAListItem ({ activityRef, refData, allRefs, style, settings, styles, onPress, onAddReference, onRemoveReference }) {
  const [reference, setReference] = useState()
  useEffect(() => {
    motaFindOneByReference(activityRef).then(setReference)
  }, [activityRef])
  ...
  return (
    <Ham2kListItem style={{ paddingRight: styles.oneSpace * 1 }}
      ...
      description={reference?.ref ? [reference?.name, reference?.type].filter(x => x).join(', ') : 'Unknown Mill Reference'}
      onPress={onPress}
