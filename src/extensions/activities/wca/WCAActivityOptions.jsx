...
export function WCAActivityOptions ({ styles, operation, settings, refs: allRefs, setRefs }) {
  ...
  useEffect(() => {
    setTimeout(async () => {
      ...
      for (const ref of activityRefs) {
        const result = await wcaFindOneByReference(ref.ref)
  ...
  useEffect(() => {
    setTimeout(async () => {
      if (location?.lat && location?.lon) {
        const newResults = await wcaFindAllByLocation(ourInfo.dxccCode, location.lat, location.lon, NEARBY_DEGREES)
        setNearbyResults(
  ...
  useEffect(() => {
    setTimeout(async () => {
      if (search?.length > 2) {
        let newRefs = await wcaFindAllByName(ourInfo?.dxccCode, search.toLowerCase())
        if (location?.lat && location?.lon) {
        ...
        if (newRefs.length > 15) {
          setResultsMessage(t('extensions.wca.activityOptions.nearestMatches', 'Nearest {{limit}} of {{count}} matches', { limit: 15, count: newRefs.length }))
        } else {
          setResultsMessage(t('extensions.wca.activityOptions.matchingCastles', '{{count}} matching castles', { count: newRefs.length }))
        }
      } else {
        setResults(nearbyResults)
        if (nearbyResults === undefined) setResultsMessage(t('extensions.wca.activityOptions.searchForCastles', 'Search for some castles to activate!'))
        else if (nearbyResults.length === 0) setResultsMessage(t('extensions.wca.activityOptions.noCastlesNearby', 'No castles nearby'))
        else setResultsMessage(t('extensions.wca.activityOptions.nearbyCastles', 'Nearby castles'))
      }
    })
  ...
  return (
    <>
      ...
      <H2kListRow>
        <H2kSearchBar
          placeholder={t('extensions.wca.activityOptions.searchPlaceholder', 'Castles by name or reference…')}
          value={search}
