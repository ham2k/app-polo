...
export function ELAActivityOptions({ styles, operation, settings, refs: allRefs, setRefs }) {
  ...
  useEffect(() => {
    setTimeout(async () => {
      if (location?.lat && location?.lon) {
        const newResults = await elaFindAllByLocation(ourInfo.dxccCode, location.lat, location.lon, NEARBY_DEGREES)
        setNearbyResults(
  ...
  useEffect(() => {
    setTimeout(async () => {
      if (search?.length > 2) {
        let newRefs = await elaFindAllByName(ourInfo?.dxccCode, search.toLowerCase())
        if (location?.lat && location?.lon) {
  ...
  return (
    <>
      ...
      <H2kListRow>
        <H2kSearchBar
          placeholder={t('extensions.ela.activityOptions.searchPlaceholder', 'Name or reference…')}
          value={search}
