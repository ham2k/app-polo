...
import { TOTAListItem } from './TOTAListItem'
import { totaFindAllByLocation, totaFindAllByName, totaFindOneByReference } from './TOTADataFile'

export function TOTAActivityOptions ({ styles, operation, settings, refs: allRefs, setRefs }) {
  ...
  useEffect(() => {
    setTimeout(async () => {
      if (location?.lat && location?.lon) {
        const newResults = await totaFindAllByLocation(location.lat, location.lon, NEARBY_DEGREES)
  ...
  useEffect(() => {
    if (search?.length > 2) {
      setTimeout(async () => {
        let newResults = await totaFindAllByName(search.toLowerCase())

        if (location?.lat && location?.lon) {
          newResults = newResults.map(ref => ({
            ...ref,
        ...
        if (newResults.length > 15) {
          setResultsMessage(t('extensions.tota.activityOptions.nearestMatches', 'Nearest {{limit}} of {{count}} matches', { limit: 15, count: newResults.length }))
        } else {
          setResultsMessage(t('extensions.tota.activityOptions.matchingTowers', '{{count}} matching towers', { count: newResults.length }))
        }
      })
    } else {
      setResults(nearbyResults)
      if (nearbyResults === undefined) setResultsMessage(t('extensions.tota.activityOptions.searchForTowers', 'Search for a tower to activate!'))
      else if (nearbyResults.length === 0) setResultsMessage(t('extensions.tota.activityOptions.noTowersNearby', 'No towers nearby'))
      else setResultsMessage(t('extensions.tota.activityOptions.nearbyTowers', 'Nearby towers'))
    }
  ...
  return (
    <>
      ...
      <H2kListRow>
        <H2kSearchBar
          placeholder={t('extensions.tota.activityOptions.searchPlaceholder', 'Name or reference…')}
          value={search}
