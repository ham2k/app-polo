...
import { llotaPrefixForDXCCCode } from './LLOTAAllRefsData'
import LLOTAInput from './LLOTAInput'

const MATCH_SYMBOLS_REGEX = /[ ,.]+$/

export function LLOTALoggingControl (props) {
  ...
  const defaultPrefix = useMemo(() => {
    if (qso?.their?.guess?.dxccCode) {
      return llotaPrefixForDXCCCode(qso?.their.guess.dxccCode) ?? '?'
    } else if (ourInfo?.dxccCode) {
      return llotaPrefixForDXCCCode(ourInfo?.dxccCode) ?? '?'
    } else {
      return '?'
    }
  ...
  return (
    <LLOTAInput
      ...
      contentStyle={[]}
      value={innerValue}
      fieldId="llotaReference"
      label={t('extensions.llota.loggingControl.theirRefLabel', 'Their LLOTA')}
